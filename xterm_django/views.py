...
from xterm_django.websocket_handler import WebSocketHandler
from xterm_django.utils.data_type import ClientData
from xterm_django.utils.ioloop import IOLoop
from xterm_django import exceptions
...
def index_view(request):
    return render(request, 'xtermindex.html')
...
def ssh_with_websocket(request):
    ...
    if not request.is_websocket():
        ...
        except exceptions.SSHShellException as e:
            LOG.debug(e.message)
            LOG.debug(type(e.message))
            code = 4001
...
def _send_message(message, ws_handler):
    ...
    client_data = ClientData(message.decode('utf-8'))
