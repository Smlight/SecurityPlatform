...
class Bridge(object):
    ...
    def destroy(self, code=1000, reason=None):
        ...
        if not reason:
            self.websocket_handler.request.websocket.close(code=code, reason=b"Connection closed by daemon.")
        else:
            self.websocket_handler.request.websocket.close(code=code, reason=reason.encode('utf-8'))
